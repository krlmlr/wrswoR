...
\newcommand{\pairadice}{%
  \raisebox{-0.5ex}{\includegraphics[height=2ex,width=!]{pair_of_dice}}}
...
(A pair of dice indicates random draws.)
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \ENSURE Returns a vector $k_j \in \{1,\ldots,n\}$ for
      $j \in \{1,\ldots,s\}$ that contains the indexes of the items sampled
    \STATE \pairadice{}
      $k_i \leftarrow \text{unique}(\text{sample}(n, \text{expected.items}(n, s), \TRUE, p_i))$
...
that operates on the logarithmic scale for increased numeric stability.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \ENSURE Returns a vector $k_j \in \{1,\ldots,n\}$ for
      $j \in \{1,\ldots,s\}$ that contains the indexes of the items sampled
    \STATE \pairadice{}
      $r_i \leftarrow \text{Exp}(1) / p_i$ for all $i \in \{1,\ldots,n\}$
...
It allows for almost trivial parallelization,
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \STATE \pairadice{}
      Set keys for these elements based on their weight and one random number per item
    \WHILE{not all items processed}
      \STATE Choose item with lowest key in the reservoir
        \label{alg:sample-expj:pq}
      \STATE \pairadice{}
        Determine number of items to skip, based on this key and a random number
      \STATE Replace item with lowest key with current item in the reservoir
      \STATE \pairadice{}
        Set the new item's key based on its weight and a random number
    \ENDWHILE
...
In the package, the functions are prefixed with \code{sample_int_}.
...
See also the documentation in \cref{pkg:wrswoR} for details.
For testing the new routines against the \proglang{R} implementation,
a wrapper function \code{sample_int_R()} is provided,
which calls the base \proglang{R} function \code{sample.int()}
with \code{replace = FALSE}:
```{r, echo=FALSE}
sample_int_R
```
...
for $n \approx \num{10000}$, in many cases even for much smaller inputs.
...
package if the number of items to choose from is just above \num{10000},
this threshold is below \num{500} for reservoir sampling with exponential jumps.
