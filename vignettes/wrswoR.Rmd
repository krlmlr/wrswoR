...
\Cref{fig:run-time-log} presents an overview of the runtime measurements
...
As expected, the *expj* algorithm appears to run fastest, especially for $r \ll 1$.
...
The *rej* algorithm exhibits initial costs on the sub-millisecond scale
even for small input sizes, probably due to the fact that it is implemented
purely in \proglang{R}.
In addition, it is by far the slowest
