...
with non-uniform weight distribution.
...
\fxwarning{Put in context of Tillé's book (start with chapter 3)}
...
It is easy to see that the entire weight vector must be read at least once.
...
The last two are based on arithmetic transformation of a weight distribution.
...
\fxwarning{Reference? Yates (1960) p. 347 contains this, as well as Tillé 3.8}
...
- *Uniform*
- *Linear*: Sequence from $1$ to $n$ ($\{p_i\} = \{1,\ldots n\}$),
...
break_even %>%
  ...
  ggplot(aes(x=n, y=R_vs_expj, color=r, group=r)) +
  ggplot_base +
  geom_line() +
  scale_x_log10(name = "$n$") +
  scale_y_log10(name = "Ratio of median run times\n(\\proglang{R} vs. \\emph{expj})", breaks = 2 ** (-1:5)) +
  scale_color_continuous(name = "$r$", trans="log", breaks = 10 ** (-2:0))
```


```{r point-break-even, echo=FALSE, fig.cap="Break-even point of \\proglang{R} and \\emph{expj} run times", dependson=c("break-even-base", "ggplot-base")}
break_even %>%
  filter(expr %in% c("R", "expj")) %>%
...
```{r correctness-true, echo=FALSE, dependson="def-plot-p-values", fig.cap="CDF of the p-values when comparing the \\emph{crank} and \\proglang{R} algorithms"}
p_values_true %>%
...
```{r correctness-false, echo=FALSE, dependson="def-plot-p-values", fig.cap="CDF of the p-values when comparing the \\proglang{R} implementation with a skewed version of itself"}
p_values_false %>%
  rename(`p-value` = p_value) %>%
  arrange(`p-value`) %>%
  mutate(`Rank` = seq_along(`p-value`)) %>% 
...
cut_p <- function(x) {
  stopifnot(x >= 0)
  stopifnot(x <= 1)
  x <- cut(x, p_value_breakpoints)
  x[is.na(x)] <- cut(p_value_breakpoints[[2L]], p_value_breakpoints)
...
comprehensive_base <- function(by) list(
  ...
  scale_fill_brewer(name = "p-value", palette = "YlOrRd"),
  NULL
)
```
...
wrswoR.benchmark::p_values_agg_agg %>%
  ...
  ggplot(aes(x=N, y=ifelse(skew == 1, func, round(skew - 1, 6)), fill=cut_p(p_value))) +
  ggplot_base +
...
```{r comprehensive-true, echo=FALSE, fig.cap="CDF of the p-values when comparing five algorithms, where one of which uses different weights", dependson = "comprehensive-base"}
wrswoR.benchmark::p_values_agg %>%
  filter(skew == 1 & func != "R") %>%
  ggplot(aes(x=N, y=n, fill=cut_p(p_value))) +
  ggplot_base +
  comprehensive_base(3) +
  scale_y_continuous(expand = c(0, 0)) +
  facet_wrap(~func)
```

```{r comprehensive-false, echo=FALSE, fig.cap="CDF of the p-values when comparing five algorithms, where one of which uses different weights", dependson = "comprehensive-base"}
wrswoR.benchmark::p_values_agg %>%
  filter(func == "R") %>%
  ggplot(aes(x=N, y=n, fill=cut_p(p_value))) +
  ggplot_base +
...
- Faster heap (g-heap?) \fxwarning{Where is that used? Not mentioned before}
- Generic C library
