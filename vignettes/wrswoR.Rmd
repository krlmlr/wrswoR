...
This can be tested with a Kolmogorov–Smirnov test against the uniform distribution
and inspecting or recomputing those experiments with the lowest p-values
for the K-S test.
...
p_values_false <- aggregated_prop_test(
  n,
  s,
  kimisc::nlist(probs$rexp(n), probs$uniform(n)),
  N,
  M,
  sample_int_R)
```
...
the K-S test cannot reject the null hypothesis of uniformity
($p = `r round(ks.test(p_values_true, punif)$p.value, 3)`$).
...
Here, the K-S test computes a much lower p-value
($p = `r round(ks.test(p_values_false, punif)$p.value, 3)`$).

```{r correctness-true, echo=FALSE, fig.cap="CDF of the p-values when comparing the five algorithms"}
p_values_true %>%
  ftable %>%
  as.data.frame %>%
  rename(`p-value` = Freq) %>%
...
```{r correctness-false, echo=FALSE, fig.cap="CDF of the p-values when comparing different weight distributions"}
p_values_false %>%
  ftable %>%
  as.data.frame %>%
  rename(`p-value` = Freq) %>%
