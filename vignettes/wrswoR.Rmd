...
- Results of comparing all five implementations at once are inconclusive
- Implement pairwise comparison
    - Comparing R vs. R worked -- branch feature/pairwise in wrswoR.correctness
    - Need to compare R with all other methods
        - Need more space, installed Git LFS on gitlab.ivt
        - Converted repos to LFS using BFG
        - Pushed sample and correctness repos using LFS to gitlab.ivt
        - Already implemented git rsync to sync Git repos to server machines
    - Run experiments
        - Fisher's exact test is better (produces lower p-values, has greater power power), but way too slow
          (needs density of hypergeometric for support of size in the order of the cell's count)
...
\Cref{fig:correctness-true} shows a Schweder plot @Schweder (essentially
a transposed empirical CDF) of the p-values resulting from an experiment that
...
p_values_true %>%
  ...
  ggplot(aes(x = `Rank`, y = `p-value`, color = i, shape = j)) +
  ggplot_base +
  geom_point() +
  geom_abline(slope = 1 / nrow(p_values_true), intercept = 0, linetype = 3) +
  coord_fixed(ratio = nrow(p_values_true))
```
...
p_values_false %>%
  ...
  ggplot_base +
  geom_point() +
  geom_abline(slope = 1 / nrow(p_values_false), intercept = 0, linetype = 3) +
  coord_fixed(ratio = nrow(p_values_false))
```
...
```{r comprehensive, echo=FALSE, dependson=character(), fig.cap="Combining p-values for a comprehensive test"}
...
```{r comprehensive-true, echo=FALSE, dependson=character(), fig.cap="CDF of the p-values when comparing five algorithms, where one of which uses different weights"}
...
```{r comprehensive-false, echo=FALSE, dependson=character(), fig.cap="CDF of the p-values when comparing five algorithms, where one of which uses different weights"}
wrswoR.benchmark::p_values_agg %>%
  filter(func == "R") %>%
  ggplot(aes(x=factor(N), y=n, fill=p_value)) +
  ggplot_base +
