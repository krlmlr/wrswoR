...
```{r run-time-log, echo=FALSE, fig.height=7, fig.cap="Median run times", dependson=c("timing-base", "ggplot-base")}
timings %>%
...
break_even %>%
  ...
  geom_line() +
  scale_x_log10() +
  scale_y_log10(name = "Ratio of median run times (R vs. expj)", breaks = 2 ** (-1:5)) +
  scale_color_continuous(trans="log", breaks = 10 ** (-2:0))
```
...
wrswoR.benchmark::p_values_agg_agg %>%
  ggplot(aes(x=factor(N), y=interaction(func, skew), fill=cut_p(p_value))) +
  ggplot_base +
  geom_tile() +
...
wrswoR.benchmark::p_values_agg %>%
  ...
  ggplot(aes(x=factor(N), y=n, fill=cut_p(p_value))) +
  ggplot_base +
  geom_tile() +
