...
(For consistency, the term *weight* is used throughout this paper for denoting
...
- If all weights $p_i$ are equal, the selection probability $\mathrm{P}(i=k)$
  of the sampled items in line \ref{alg:sample:norm} always equals $\frac{1}{n}$
  and does not have to be computed explicitly.

In this framework, sampling without replacement with non-uniform weights
...
is also the lower bound for arbitrary weight distributions:
The entire weight vector must be read at least once.
...
In addition to drawing random numbers, the extraction of the minimal item
...
The subsequent sections show performance characteristics and correctness
...
\begin{description}
  ...
  *ascending* ($\nearrow$), *descending* ($\searrow$) and *shuffled* ($\leadsto$)
...
relabel_expr <- function(x) {
  factor(x, labels = format_expr(levels(x)))
}
```
...
with 12 MB cache,
using version 0.4 of the \pkg{wrswoR} package.
...
and can only be explained with run time differences between partial sort
(which is used for *crank*) and priority queue (for *expj*) implementations.

```{r run-time-crank-expj, echo=FALSE, fig.cap="Comparison of \\emph{crank} and \\emph{expj} run times", dependson=c("timing-base", "ggplot-base")}
timings %>%
...
The worst slowdown of the *expj* algorithm seems to be around 2,
for absolute run times of around 10 microseconds for $n = 100$.
It is remarkable that the worst slowdown occurs with $r = 0.1$
in this plot.
...
even for $s = 1$,
which does not occur in the *expj* implementation.


```{r run-time-break-even, echo=FALSE, fig.cap="Comparison of \\proglang{R} and \\emph{expj} run times for \\emph{linear ascending} weights", dependson=c("break-even-base", "ggplot-base")}
break_even %>%
  ...
  ggplot(aes(x=n, y=R_vs_expj, color=factor(r))) +
  ggplot_base +
  geom_line() +
...
for a larger range of $r$ and for all weight distributions tested.
...
break_even_point %>%
  ...
  scale_y_log10(name = "$n$", breaks = c(50, 100, 200, 400)) +
...
```{r correctness-true, echo=FALSE, dependson="def-plot-p-values", fig.cap="Schweder plot for p-values resulting from comparing the \\emph{crank} and \\proglang{R} algorithms"}
p_values_true %>%
...
```{r correctness-false, echo=FALSE, dependson="def-plot-p-values", fig.cap="Schweder plot for p-values resulting from comparing the \\proglang{R} implementation with a skewed version of itself"}
p_values_false %>%
  rename(`p-value` = p_value) %>%
  arrange(`p-value`) %>%
  mutate(`Rank` = seq_along(`p-value`)) %>% 
...
algorithms, and the stock implementation with and without altered weights
...
wrswoR.benchmark::p_values_agg_agg %>%
  ungroup %>%
  mutate(facet = kimisc::ofactor(ifelse(skew == 1, "skew 0", "\\proglang{R}"))) %>%
...
Therefore, it is helpful to further disaggregate the p-values.
\Cref{fig:comprehensive-true} shows combined p-vaules separately for all pairs
of $n$ and $N$ when comparing the new algorithms to the stock implementation.
...
```{r comprehensive-true, echo=FALSE, fig.cap="Combined p-values for different values of $n$ and $N$, resulting from comparing the new algorithms to the stock implementation", dependson = "comprehensive-base"}
wrswoR.benchmark::p_values_agg %>%
  ungroup %>% 
  filter(skew == 1 & func != "R") %>%
  mutate(func = format_expr(func)) %>%
...
of the stock implementation in the most favorable case,
...
The validation methodology is able to clearly detect a simulated implementation
...
Calling `R_CheckUserInterrupt()` every $10^7$ or so operations
...
probabilities, and the order is not important, as e.g. in the application of
...
the plots were created using \pkg{ggplot2}
