...
```{r prob-dist, echo=FALSE, fig.height=3, fig.cap="More shit"}
prob_data %>%
  filter(.id != "shuffled_linear") %>%
  ggplot(aes(x, y, linetype=.id)) +
  ggplot_base +
  geom_line() +
...
For $n = `r max(timings$n)`$, the mean run time of the \proglang{R} implementation
...
- rej doesn't work for highly skewed distributions
