...
The paper concludes with suggestions for incorporating the findings into
\proglang{R}.
...
```{r overhead-sorting, fig.cap="Overhead of unsorted data vs.\\ sorting costs", echo=FALSE, dependson=c("ggplot-base")}
median_timings <-
  timings %>%
...
timings_filter <- . %>%
  ...
  filter(mix != "$\\nearrow$") %>% 
  filter(n > 10000) %>%
  mutate(n = kimisc::ofactor(paste0("$n = ", n, "$")))
