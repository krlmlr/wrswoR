...
- This entire procedure is repeated $M$ times,
  this results in $M$ p-values for each tuple $(i, j)$.
...
This can be tested by comparing a quantile of the p-values
with a threshold: For instance, if the 5%-quantile is lower than 0.01,
it is very likely that one of the implementations is faulty.
...
The above procedure does not constitute a proof of correctness.
Nevertheless, it offers a means to automatically test the implementations for
nontrivial errors.
...
```{r echo=FALSE}
  probs <- wrswoR.benchmark::prob(alpha)
  p_values_true <- aggregated_prop_test(
    n,
    s,
    probs$rexp(8),
    N,
    M,
    kimisc::nlist(sample_int_R, sample_int_rej, sample_int_rank, sample_int_crank, sample_int_expj))
```

\Cref{fig:correctness-true} shows a plot of the p-values against their rank
when drawing $M = `r M`$ times $N = `r N`$ samples for $n = `r n`$,
...
the 5%-quantile is at `r round(quantile(p_values_true, 0.05), 3)`,
far above the 0.01 threshold suggested before.

For an exhaustive test for all
$n \in \{`r min(as.numeric(wrswoR.benchmark::p_values_df$n))`, \ldots,
`r max(as.numeric(wrswoR.benchmark::p_values_df$n))`\}$,
all $s \in \{1, \ldots, n\}$
and all probability distributions discussed above (with $\alpha = 2$),
the 5%-quantiles of the p-values were between 



```{r correctness-true, echo=FALSE, fig.cap="Sorted p-values when comparing the five algorithms"}
  p_values_true %>%
    ftable %>%
    as.data.frame %>%
    rename(`p-value` = Freq) %>%
    arrange(`p-value`) %>%
    mutate(Rank = (seq_along(`p-value`) - 1) / (length(`p-value`) - 1)) %>% 
...
```
  ...
  ```{r echo=FALSE}
  p_values_false <- aggregated_prop_test(
    10,
    5,
...
- Perform tests with larger ranges: Accumulate 5%-quantiles of p-values distribution
