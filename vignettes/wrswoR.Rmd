...
\fxwarning{double-check complexity}
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \ENSURE Returns a vector $k_j \in \{1,\ldots,n\} $ with
      $j \in \{1,\ldots,s\}$ that contains the indexes of the items sampled
    \STATE $k_i \leftarrow \text{unique}(\text{sample}(n, \text{expected.items}(n, s), \FALSE, p_i))$
...
\fxwarning{cite radix sort}.
However, generating random variates is computationally expensive,
and for $s \ll n$ this may dominate the actual run time cost.
\fxnote{Parallelize}.
...
Compared to \code{sample.int}, the name of the base \proglang{R} function,
