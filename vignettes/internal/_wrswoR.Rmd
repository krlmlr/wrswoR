...
based on a given sampling design.
In the context of that framework, \cref{alg:sample} belongs to the class of "draw by draw" algorithms.
...
an implementation in \proglang{C} seems necessary.
Other platforms for scientific computing, such as \proglang{Python} or \proglang{Julia},
would also benefit if this implementation was provided in an open-source library with a documented interface.
...
The algorithms presented here generate an ordered sample of items based on relative weights.
If the relative importance is instead given as inclusion probabilities, and the order of the items is irrelevant, e.g.,
as in the application of survey sampling, the `UPxxx()` functions in the \pkg{sampling} package
